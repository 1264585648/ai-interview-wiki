...
[https://nfturbo-file.oss-cn-hangzhou.aliyuncs.com/llm/%E6%B5%8B%E8%AF%95%E7%94%A8%E6%95%B0%E6%8D%AE.zip](https://nfturbo-file.oss-cn-hangzhou.aliyuncs.com/llm/%E6%B5%8B%E8%AF%95%E7%94%A8%E6%95%B0%E6%8D%AE.zip)

多版本支持的数据模型设计

设计思路：逻辑文档与物理版本分离

之前我们设计的只有knowledge_document的单表方案在面对版本管理时会遇到诸多问题：每次更新都会覆盖原始数据，无法回溯；切换版本需要删除再重建，存在服务中断窗口。
...
CREATE TABLE `knowledge_segment` (
    ...
    `document_id`      BIGINT NOT NULL COMMENT '逻辑文档 ID',
    `document_version` BIGINT COMMENT '版本 ID（精确绑定到某个版本）',
    `text`             LONGTEXT,
    `embedding_id`     VARCHAR(255) COMMENT 'ES 向量 ID',
    `status`           VARCHAR(255) COMMENT 'STORED / VECTOR_STORED',
...
![](assets/7eea2fa92fda.png)

同一份文档，如果有新的版本，可以通过上传新版本的方式更新文档内容。流程如下：

![](assets/343a6f40974f.svg)
...
| knowledge_document_version | 新增 1 条新版本记录(2.0.0)；status 从 UPLOADED 演进至 VECTOR_STORED；<br>旧版本记录保留(1.0.0)，但是状态会变为CHUNKED |
| knowledge_segment | 新增新版本(2.0.0)对应的 N 条片段；新版本片段 status 从 STORED → VECTOR_STORED 并回填 embedding_id；<br>旧版本片(1.0.0)段保留，但在 embedAndStore 的双检阶段被降级为 STORED 并清空 embedding_id |
| Elasticsearch 向量库 | 写入新版本向量；(2.0.0)<br>旧版本(1.0.0)向量在 embedAndStore 中通过 deactivateVersion 按 docId + versionId 删除 |
...
![](assets/b5e9eb8db668.png)

主要流程如下：

![](assets/cd97a4f4b251.svg)
...
| knowledge_document_version | 目标版本(1.0.0) status 由 CHUNKED 升为 VECTOR_STORED；<br>原版本(2.0.0) status 由 VECTOR_STORED 降为 CHUNKED |
| knowledge_segment | 目标版本(1.0.0)片段 status 从 STORED → VECTOR_STORED 并回填 embedding_id；<br>原版本(2.0.0)片段 status 从 VECTOR_STORED 降为 STORED 并清空 embedding_id |
...
knowledgeSegmentMapper.update(null, segUpdate);
...
knowledgeSegmentMapper.update(null, updateWrapper);
...
来源: https://thoughts.aliyun.com/workspaces/6963289eb0fc2e001bb052eb/docs/6a36c28cd31fed0001a4cdf0
