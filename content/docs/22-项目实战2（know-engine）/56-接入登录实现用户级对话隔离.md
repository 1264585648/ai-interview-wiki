...
public LoginUserVO login(LoginDTO loginDTO) {
    ...
    // 5. 调用 sa-token 建立会话
    StpUtil.login(userInfo.getId());  // loginId = user_info.id

    log.info("客户登录成功: phone={}, name={}, loginId={}", 
             userInfo.getPhone(), userInfo.getName(), userInfo.getId());
...
if (data.code === 200) {
    // sa-token 已通过 Cookie 下发 token，跳转到聊天页
...
StpUtil.login(STAFF_LOGIN_PREFIX + staffInfo.getId(), STAFF_DEVICE);
...
public LoginUserVO getCurrentUser() {
    ...
    if (loginId.startsWith(STAFF_LOGIN_PREFIX)) {
        Long staffId = Long.parseLong(loginId.substring(STAFF_LOGIN_PREFIX.length()));
...
.notMatch("/auth/**")        // 认证相关接口
.notMatch("/dingtalk/**")    // 钉钉回调接口
.notMatch("/druid/**")       // 数据库监控
```
...
SaRouter.match("/api/document/**", "/api/segment/**")
        .check(r -> {
            ...
            String loginId = StpUtil.getLoginIdAsString();
            if (!loginId.startsWith("staff_")) {
                throw new NotLoginException("文档管理需要员工登录", 
                                            NotLoginException.NOT_TOKEN, null);
            }
        });
```
...
public List<ChatConversation> listConversations() {
    String userId = authService.getCurrentUserId();  // 自动从 sa-token 获取
...
来源: https://thoughts.aliyun.com/workspaces/6963289eb0fc2e001bb052eb/docs/6a3fa86b74e4030001ca80c9
