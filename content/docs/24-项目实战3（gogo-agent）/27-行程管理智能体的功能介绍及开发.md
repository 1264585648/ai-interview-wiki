...
| --- | --- | --- | --- |
| **提交新申请** | query_user_base_location<br>→<br>check_travel_order_conflicts<br>→<br>submit_travel_approval | 冲突检测 | 信息确认 + 冲突确认(HIGH) |
| **查询行程/审批** | query_travel_order<br>/<br>query_approval_status | 无 | 无 |
| **取消申请** | query_travel_order<br>→<br>cancel_travel_order<br>→<br>cancel_booking<br>(关联) | 状态校验 | 不可恢复确认 + APPROVED 二次确认 |
| **修改申请** | query_travel_order<br>→<br>check_travel_order_conflicts<br>→<br>modify_travel_order<br>→<br>cancel_booking<br>(关联) | 冲突检测 + 状态校验 | 变更确认 + APPROVED 二次确认 |
