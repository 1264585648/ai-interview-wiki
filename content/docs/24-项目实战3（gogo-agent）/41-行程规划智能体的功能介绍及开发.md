...
| 五维结构化审核 | ItineraryReviewTools 工具 | 客观+主观审核 |
...
⑦ 审核     → ItineraryReviewTools（主观+客观检查）
...
- **审核工具**：负责行程的整体把关，包括主观审核(Agent)+客观审核（工具）

User Case

![](assets/afe456cec21a.png)

![](assets/05a743b54652.png)

![](assets/be94776a95d8.png)
...
即使不修复也需 ~15 轮工具调用，留 30 轮可覆盖 2 次修复。
...
**审核这个部分我们做过优化和改造，原来使用的智能体，后来改成了工具，在工具中做智能体的编排。**
...
ItineraryPlanAgent 注册了 14 个 Hook：

| Hook | 作用 |
| --- | --- |
| DynamicTimeInjectionHook | 注入当前时间（保持 system prompt 前缀稳定以利用缓存） |
| AutoContextHook | 上下文超 128K 时自动压缩旧消息 |
| PlanAwareThinkingHook | 按 Plan/Execute/Summary 阶段切换 thinking |
| CliResultCompressHook | CLI 返回结果去重+删冗余字段 |
...
| ToolCircuitBreakerHook | Redis + Lua 熔断器 |
