...
官方 Skill 是**面向"单机 + 桌面 Agent（Cursor / Claude Desktop/龙虾/work buddy）"** 编写的说明书，直接搬进服务端有四类不匹配：
...
---
name: tuniu-cli
description: 途牛旅行统一助手- 通过 tuniu CLI 统一调用机票、酒店、火车票等旅行服务。
             适用于用户询问航班、酒店、火车票相关需求的场景。只针对国内酒店、机票、火车票的查询和预定。
version: 1.0.4
minCliVersion: 1.0.7
---
```
