...
**审批检查（强制，不可跳过）** ：执行预订前，必须调用 `check_travel_order_approval` 检查是否存在已审批通过的差旅单：   - 若返回 `approved=true` ：可执行预订，从返回结果中获取 `order_id`、`destination` 等信息用于后续下单。   - 若返回 `approved=false` ：**直接拒绝预订** ，根据 `message` 告知用户"差旅单尚未通过审批，无法执行预订。请先提交差旅申请并等待审批通过后再预订。"，不得以任何理由绕过此检查。
