...
public class TravelOrderConflictTools {
    ...
    @Autowired private CityTransitTimeService cityTransitTimeService;
    ...
    public String checkTravelOrderConflicts(
            ...
            @ToolParam(name = "exclude_order_id",
                    description = "需要排除的差旅单ID（修改场景下排除自身），可选",
                    required = false) String excludeOrderId) {

        String userId = sessionCtx.getUserId();

        // ── 入参校验（错误信息拼成一句话回传 LLM）──────────────────────────
        List<String> errors = new ArrayList<>();
        if (isBlank(userId))         errors.add("user_id 不能为空");
        if (isBlank(departureCity))  errors.add("departure_city（出发城市）不能为空");
        if (isBlank(destination))    errors.add("destination（目的地）不能为空");
        if (!isValidDate(departureDate)) errors.add("departure_date 格式错误或为空，需 YYYY-MM-DD");
        if (!isValidDate(returnDate))    errors.add("return_date 格式错误或为空，需 YYYY-MM-DD");
        if (!errors.isEmpty()) {
        ...
        if (depDate.isAfter(retDate)) {
            return JSON.toJSONString(new ConflictReport() {{
                setHasConflict(false);
                setSummary("出发日期晚于返回日期，请先修正日期。");
            }});
        }

        // ── 拉候选：已有生效中差旅单，且日期区间与候选有交叠 ─────────────
...
```text
1. **发起新差旅前，必须调用 `check_travel_order_conflicts(...)` 检查冲突**。该工具覆盖两类检查：
   - 同一员工时间重叠（同城=LOW重复提交，跨城=HIGH物理上不可能）；
   - 跨城市交通衔接合理性（同日衔接时间不足 / 跨城但未衔接）。
2. 若用户未提供出发城市，在调用 `check_travel_order_conflicts` 前/后，
   调用 `query_user_base_location` 补全 base 地信息。
3. 工具返回 `has_conflict=true` 时，按严重等级处理：
   - **HIGH**（物理上不可能/同日衔接超过 24h）：必须先向用户展示冲突明细并征得用户明确同意后才能提交
     （用户回复"确认提交/同意忽略冲突"等明确语义）。
...
```text
3. 当修改会影响日期或城市时（出发日期/返回日期/出发城市/目的地任一变更），必须先调用 `check_travel_order_conflicts` 重新检测冲突，并在用户确认修改前将冲突明细告知用户：
