...
| **SkillsReactAgent** | `AgentStreamEvent` | **text、thinking、tool_start、tool_end、error、complete** |
...
for (AssistantMessage.ToolCall tc : toolCalls) {
    Schedulers.boundedElastic().schedule(() -> {
        ...
        // 1. 发送 ToolStart 事件（工具开始执行）
        sink.tryEmitNext(new AgentStreamEvent.ToolStart(toolName, tc.id(), argsJson).toJSON());

        ToolCallback callback = findTool(toolName);
        if (callback == null) {
            // 工具未找到 → ToolEnd 带错误
            sink.tryEmitNext(new AgentStreamEvent.ToolEnd(toolName, tc.id(),
                    "{ \"error\": \"工具未找到：" + toolName + "\" }").toJSON());
            // ...
            return;
        }
        ...
        try {
            ...
            // 3. 发送 ToolEnd 事件（工具执行失败，但结果中包含错误信息）
            sink.tryEmitNext(new AgentStreamEvent.ToolEnd(toolName, tc.id(),
                    "{ \"error\": \"工具执行失败：" + ex.getMessage() + "\" }").toJSON());
        }
    });
}
```
...
**RoundState 中的 mode 字段**是判断本轮是否有工具调用的关键。`processChunk` 中一旦发现 tool_call，就把 `mode` 切换为 `RoundMode.TOOL_CALL`。如果整轮结束时 mode 不是 `TOOL_CALL`，说明模型直接给出了最终回答。
...
Disposable disposable = chatClient.prompt()
        ...
        .onErrorResume(err -> {
            if (retryAttempt < maxRetries) {
                // 还能重试 → 发送 Error 事件，等 RETRY_INTERVAL_MS 后重试
                ...
                // 重试次数用尽 → 发送 Error 事件 + Complete 事件，结束流程
                sink.tryEmitNext(new AgentStreamEvent.Error("LLM_CALL_FAILED",
...
| 事件类型 | `Error`<br>事件 | `ToolEnd`<br>事件（result 中包含错误） |
| 重试方式 | 代码层面自动重试（<br>`maxRetries`<br>） | 依赖 React 循环，LLM 自主修复 |
| 前端展示 | 时间线中显示错误+重试状态 | 工具调用状态标记为完成 |
| 重试次数 | `maxRetries`<br>参数控制（默认 3） | 由 LLM 决定，受<br>`maxRounds`<br>限制 |
...
| `tool_end` | 更新对应条目的<br>`status: 'completed'` | 工具名 + 完成标记 |
...
const processStreamData = (data, aiMsg, thinkingContent) => {
    if (data.type === 'text' && data.content) {
        ...
        updateStreamContent(aiMsg.content);              // 实时渲染 Markdown
    } else if (data.type === 'thinking' && data.content) {
        aiMsg.hasThinking = true;
        aiMsg.thinking.push(data.content);
        // 追加到时间线
    } else if (data.type === 'tool_start') {
        aiMsg.timeline.push({type: 'tool', toolName: '...', status: 'running'});
    } else if (data.type === 'tool_end') {
        // 找到对应的 tool_start，更新状态为 completed
    } else if (data.type === 'error') {
        aiMsg.timeline.push({type: 'error', message: '...'});
...
![](assets/d4dc2586d0ba.gif)

故意把大模型apikey写错，触发error：

![](assets/8d2e7e5cbb6a.gif)

---

来源: https://thoughts.aliyun.com/workspaces/6963289eb0fc2e001bb052eb/docs/69faec4a3fb91800016c83cd
