...
![image.png](assets/2f60865b4d6b.png)
...
LangChain在2025年7月发布了一篇文章《[Context Engineering for Agents](https://blog.langchain.com/context-engineering-for-agents/?spm=5176.28103460.0.0.96a07551IpB35e)》，从Agent开发的角度来看，为了让Agent的上下文恰到好处，文章将上下文工程方法归纳为四个过程：**Write（写入）、Select（选择）、Compress（压缩）、Isolate（隔离）**。

![image.png](assets/c5037be7e84f.png)
...
比如LangGraph中使用Checkpoint来实现类似的功能，每次智能体执行一个“超步”（super-step，即完成一轮节点计算），LangGraph 会自动将当前**完整状态（state）** 保存为一个 **checkpoint（检查点）**。这些 checkpoint 按 **thread_id（线程 ID）** 归档，代表一次独立的用户会话或任务实例。开发者可通过 `graph.update_state()` 主动向状态中**写入信息**（如计划、工具结果、错误日志等）。

![image.png](assets/f7c96d1289c0.png)

就像我们的Spring AI中的conversation_id一样，也是类似的方案。
...
来源: https://thoughts.aliyun.com/workspaces/6963289eb0fc2e001bb052eb/docs/699d7a74a7c8ff00015ce108
